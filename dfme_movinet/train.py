    ...
    for i in range(args.epoch_itrs):
        ...
        for _, _ in enumerate(range(args.d_iter)):
            ...
            if loss_S.item() < best_loss:
                ...
                name = 'resnet34_8x'
                torch.save(student.state_dict(),f"checkpoint/student-new-newnew_{args.model_id}/{args.num_classes}-{name}-{args.epoch_itrs}-{info_loss_coef}.pt")
                torch.save(generator.state_dict(),f"checkpoint/student-new-newnew_{args.model_id}/{args.num_classes}-{name}-{args.epoch_itrs}-{info_loss_coef}-generator.pt")
                print(args.model_id)
...
def main():
    ...
    if args.MAZE:
        ...
        optimizer_RNN = optim.AdamW([
            {'params': generator.recurrent.parameters(), 'lr':1e-4 }
        ])
