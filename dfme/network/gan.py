...
#         )
    ...
    def sample_z_categ(self, num_samples, video_len):
        ...
        print(n_samples,'n samples')
        # b = num_samples.detach().cpu().numpy().shape[1]

        classes_to_generate = np.random.randint(self.dim_z_category, size=(n_samples))
        print(classes_to_generate.shape,classes_to_generate,'classes to generate shape')
        # one_hot = np.zeros((n_samples, self.dim_z_category), dtype=np.float32)
        if classes_to_generate.shape[0]==1:
            one_hot = F.one_hot(classes_to_generate, num_classes=400).numpy()
        ...
        return Variable(one_hot_video), classes_to_generate, Variable(one_hot)
    ...
    def sample_z_video(self, num_samples, video_len=None):
        ...
        z_category, z_category_labels, z_categ_ohe = self.sample_z_categ(num_samples, video_len)
        z_category = z_category.to(self.device)
        z_categ_ohe = z_categ_ohe.to(self.device)
        ...
        return z, z_category_labels, z_categ_ohe

    def sample_videos(self, num_samples, pre_x = None, video_len=None):
        video_len = video_len if video_len is not None else self.video_length

        z, z_category_labels, z_categ_ohe = self.sample_z_video(num_samples, video_len)
        ...
        if pre_x :
            return h, Variable(z_category_labels, requires_grad=False), Variable(z_categ_ohe, requires_grad=False)
        else:
            return self.activation(h)
        # return h#, Variable(z_category_labels, requires_grad=False)

    def sample_images(self, num_samples):
        z, z_category_labels, __________ = self.sample_z_video(num_samples * self.video_length * 2)

        j = np.sort(np.random.choice(z.size(0), num_samples, replace=False)).astype(np.int64)
        z = z[j, ::]
        z = z.view(z.size(0), z.size(1), 1, 1)
        h = self.modelG(z)
        h = h.view(h.size(0) // 1, 1, self.n_channels, h.size(3), h.size(3))
