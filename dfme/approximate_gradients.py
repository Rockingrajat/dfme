...
def estimate_gradient_objective(args, victim_model, clone_model, x, fake_labels, lambd = 0.1, epsilon = 1e-7, m = 5, verb=False, num_classes=600, device = "cpu", pre_x=False):
    ...
    with torch.no_grad():
        ...
        for i in (range(N * m // max_number_points + 1)): 
            ...
            if num_classes == 600:
                ...
                pts2 = (pts+ pts_min)/(pts_max-pts_min)
                victim_model.clean_activation_buffers()
                pred_victim_pts = victim_model(pts2.permute(0,2,1,3,4).to(device)).to(device)
            else:
        ...
        ###jugaad
        for i in range(m+1):
            pred_gen.append(fake_labels)

        # print(torch.argmax(pred_victim),torch.argmax(pred_clone),torch.argmax(pred_gen))
        # print("                                       ")
        pred_victim = torch.cat(pred_victim, dim=0).to(device)
        pred_clone = torch.cat(pred_clone, dim=0).to(device)
        pred_gen = torch.cat(pred_gen, dim=0).to(device)

        #print(pred_clone.size(), '\n', pred_victim.size(), '\n', pred_gen.size())
        ...
        # Compute loss
        if args.loss == "kl":
            loss_values = - loss_fn(pred_clone, pred_victim, reduction='none').sum(dim = 1).view(-1, m + 1) + lambd * loss_fn(pred_victim, pred_gen, reduction='none').sum(dim = 1)
            #print(loss_values.size())
        else:
...
def compute_gradient(args, victim_model, clone_model, x, pre_x=False, device="cpu"):
    ...
    if args.num_classes == 600:
        ...
        pts2 = (pts+ pts_min)/(pts_max-pts_min)
        victim_model.clean_activation_buffers()
        pred_victim = victim_model(pts2.permute(0,2,1,3,4).to(device))
    else:
        pred_victim_pts = victim_model(x_.permute(0, 2, 1, 3, 4)).to(device)   
    ...
    print(torch.argmax(pred_victim),torch.argmax(pred_clone),'pred_victim and pred_clone')
