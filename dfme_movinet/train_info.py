    ...
    for i in range(args.epoch_itrs):
        ...
        for _, _ in enumerate(range(args.d_iter)):
            ...
            if loss_S.item() < best_loss:
                ...
                name = 'resnet50'
                torch.save(student.state_dict(),f"checkpoint/student_{args.model_id}_{args.num_classes}-{name}-{args.epoch_itrs}-{info_loss_coef}.pt")
                torch.save(student2.state_dict(),f"checkpoint/student2_{args.model_id}_{args.num_classes}-{name}-{args.epoch_itrs}-{info_loss_coef}.pt")
                torch.save(generator.state_dict(),f"checkpoint/student_{args.model_id}_{args.num_classes}-{name}-{args.epoch_itrs}-{info_loss_coef}-generator.pt")
...
def main():
    ...
    for epoch in tqdm(range(1, number_epochs + 1)):
        ...
        if args.store_checkpoints:
            torch.save(student.state_dict(), args.log_dir + f"/checkpoint/student-new-{epoch}-info-{lambd}.pt")
            torch.save(student2.state_dict(), args.log_dir + f"/checkpoint/student2-new-{epoch}-info-{lambd}.pt")
            torch.save(generator.state_dict(), args.log_dir + f"/checkpoint/generator-new-{epoch}-info-{lambd}.pt")
